...
async def process_line(bot, line: str):
    ...
    if "hit by" in line or "killed by" in line or "CHAR_DEBUG - KILL" in line or "died." in line:
        ...
        if not killed:
            # ────────────────────────────────────────────────
            # NOWE: Hit od Infected / Zombie / AI / zwierząt
            # ────────────────────────────────────────────────
            if "hit by" in line and not 'Player "' in line[line.find("hit by"):]:
                ...
                if match_npc_hit:
                    ...
                    if is_dead:
                        ...
                        color = "[31m"
                        extra = " (ŚMIERĆ od NPC)"
                        # Można wysłać też na kanał kill/death
                        ...
                        emoji = "🧟"
                        color = "[35m"          # fioletowy dla zombie
                        extra = f" (HP: {hp:.1f})"
                    
                    msg = f"{date_str} | {log_time} {emoji} {victim}{extra} → trafiony przez {attacker} w {part}"
                    ch = client.get_channel(CHANNEL_IDS["damages"])  # albo osobny kanał np. "pve"
                    if ch:
                        await ch.send(f"```ansi\n{color}{msg}[0m\n```")
                    return
