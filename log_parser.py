...
async def process_line(bot, line: str):
    ...
    if "is connected" in line and 'Player "' in line:
        ...
        if match:
            ...
            print(f"[PARSER] Wykryto połączenie: {name} (guid: {guid})")
            return
        else:
            print(f"[PARSER DEBUG] Linia 'is connected' nie pasuje: {line[:150]}")
    ...
    if any(x in line.lower() for x in ["disconnected", "has been disconnected", "kicked", "banned"]) and 'Player ' in line:
        ...
        if name_key in player_login_times:
            delta = datetime.utcnow() - player_login_times[name_key]
    ...
    # 4. Hit / Kill – (pozostała część Twojego kodu bez zmian)
    # ... (wklej swoją całą sekcję od if any(x in line for x in ["hit by", ...]) do końca hit player vs player)
