...
async def process_line(bot, line: str):
    ...
    # 2. Rozłączono – czerwony (poprawiony regex + bardziej elastyczny)
    ...
    # 4. Obrażenia i śmierci – żółty/pomarańczowy dla hit, czerwony tylko dla śmierci
    if any(kw in line for kw in ["hit by", "killed by", "[HP: 0]", "CHAR_DEBUG - KILL"]):
        # Najpierw sprawdzamy pełne zabójstwo (killed by)
        ...
        if match_hit:
            ...
            # Żółty/pomarańczowy dla obrażeń, czerwony tylko przy śmierci
            color = "[31m" if is_dead else "[38;5;208m"  # pomarańczowy
            emoji = "☠️" if is_dead else "⚡"

            msg = f"{date_str} | {log_time} {emoji} {victim} {'(ŚMIERĆ)' if is_dead else ''} trafiony przez {attacker} w {part} za {dmg} dmg ({ammo}) z {weapon} z {dist}m (HP: {hp})"
            ch = client.get_channel(CHANNEL_IDS["deaths"])
            if ch:
                await ch.send(f"```ansi\n{color}{msg}[0m\n```")
            return
