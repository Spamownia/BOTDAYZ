...
async def process_line(bot, line: str):
    ...
    # 4. Hit / Kill – anty-duplikaty (Twoja oryginalna wersja bez zmian)
    if "hit by" in line or "killed by" in line or "CHAR_DEBUG - KILL" in line or "died." in line:
        ...
        if not killed:
            # Rozszerzony regex – obsługuje zarówno hit by Player, jak i hit by Infected itp.
            match_hit = re.search(
                r'Player "(?P<victim>[^"]+)" \((?:id=[^)]+ pos=<[^>]+>)?\)\[HP: (?P<hp>[\d.]+)\] hit by (?P<attacker>[^ ]+)(?: "(?P<attacker_name>[^"]+)")?.*into (?P<part>\w+)\(\d+\) for (?P<dmg>[\d.]+) damage \((?P<ammo>[^)]+)\)(?: with (?P<weapon>[^ ]+) from (?P<dist>[\d.]+) meters)?',
                line
            )
            if match_hit:
                victim = match_hit.group("victim").strip()
                victim_key = victim.lower()
                attacker = match_hit.group("attacker").strip()
                if match_hit.group("attacker_name"):
                    attacker += f' "{match_hit.group("attacker_name")}"'
                part = match_hit.group("part")
                dmg = match_hit.group("dmg")
                ammo = match_hit.group("ammo")
                weapon = match_hit.group("weapon") or "brak"
                dist = match_hit.group("dist") or "?"
                hp = float(match_hit.group("hp"))
