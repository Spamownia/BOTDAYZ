...
from log_parser import process_line, reset_first_check   # ← dodajemy import funkcji resetującej
...
async def update_status():
    try:
        r = requests.get(
            f"https://api.battlemetrics.com/servers/{BATTLEMERTICS_SERVER_ID}",
            timeout=10
        )
        if r.status_code == 200:
            d = r.json()["data"]["attributes"]
            await client.change_presence(
                activity=discord.Game(f"{d['players']}/{d['maxPlayers']} online")
            )
...
async def on_ready():
    ...
    await check_logs()          # pierwsze odczytanie logów (pomija stare linie)
    
    # ← KLUCZOWA LINIA – resetujemy flagę po pierwszym przetworzeniu
    reset_first_check()
    print("[MAIN] Flaga is_first_check zresetowana – parser przetwarza teraz nowe wpisy")
